...
const ADMIN_EMAIL = process.env.NEXT_PB_ADMIN_EMAIL || 'admin@admin.admin'; // Replace
const ADMIN_PASSWORD = process.env.NEXT_PB_ADMIN_PASSWORD || 'adminadmin'; // Replace
...
async function seedDatabase(monthNum) {
    try {
        ...
        for (let i = 0; i < numberOfNoMentorShiftsToCreate; i++) {
            ...
            const shiftDate = `2025-0${monthNum}-${String(dayInJune).padStart(2, '0')}T04:00:00`;
            const shiftData = {
